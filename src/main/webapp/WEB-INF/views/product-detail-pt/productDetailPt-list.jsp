	...
	<div class="">
		...
		<div class="col-md-12 col-sm-12 col-xs-12">
			<div class="x_panel">
				...
				<div class="x_content">
					<a href="<c:url value="/product-detail-pt/add"/>" class="btn btn-app"><i class="fa fa-plus"></i>Add</a>
					<div class="container" style="padding: 50px;">
						<%--@elvariable id="searchForm" type=""--%>
						<form:form modelAttribute="searchForm" cssClass="form-horizontal form-label-left" servletRelativeAction="/product-detail-pt/list/1" method="POST">
							...
							<div class="form-group">
								<label class="control-label col-md-3 col-sm-3 col-xs-12" for="shelfName">Shelf </label>
								<div class="col-md-6 col-sm-6 col-xs-12">
									<form:input path="shelfName" cssClass="form-control col-md-7 col-xs-12" />
								</div>
							</div>
							...
							<div class="form-group">
								<div class="col-md-6 col-sm-6 col-xs-12 col-md-offset-3">
									...
									<button class="btn btn-success"><a href="<c:url value="/product-detail-pt/getAll/1"/>"  style="color: white">Get All</a></button>
							...
							<tbody>
								<c:forEach items="${products}" var="product" varStatus="loop">
									...
									<td class="text-center">
										<c:choose>
											<c:when test="${product.status=='Valid'}">
												<a href="<c:url value="/product-detail-pt/changeStatus/${product.id }"/>" class="btn btn-round btn-warning">Export</a>
											</c:when>
											<c:when test="${product.status=='InValid'}">
												<a href="<c:url value="/product-detail-pt/changeStatus/${product.id }"/>" class="btn btn-round btn-success">Reimport</a>
											</c:when>
										</c:choose>
									</td>
...
<script type="text/javascript">
	 function confirmDelete(id){
		 if(confirm('Do you want delete this record?')){
			 window.location.href = '<c:url value="/product-detail-pt/delete/"/>'+id;
		 }
	 }
	 function gotoPage(page){
		 $('#searchForm').attr('action','<c:url value="/product-detail-pt/list/"/>'+page);
		 $('#searchForm').submit();
	 }
