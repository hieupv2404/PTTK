...
public class ProductInfoService {
    ...
    public List<ProductInfo> getAllProductInfo(ProductInfo productInfo, Paging paging){
        ...
        if(productInfo!=null && productInfo.getCategory()!=null) {
            if(productInfo.getId()!=null && productInfo.getId()!=0) {
                queryStr.append(" and model.id=:id");
                mapParams.put("id", productInfo.getId());
            }
            if(productInfo.getCategory().getName()!=null && !StringUtils.isEmpty(productInfo.getCategory().getName())) {
                queryStr.append(" and model.category.name like :cateName");
                mapParams.put("cateName","%"+ productInfo.getCategory().getName()+"%");
            }
